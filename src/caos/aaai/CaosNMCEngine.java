...
import caos.aaai.NMC.NestedMonteCarlo; //remove ?
import caos.aaai.PNS.ExhaustiveSearch; // TODO
import caos.aaai.astar.Heuristic; // remove ?
import chemaxon.reaction.*;
import chemaxon.formats.*;
import chemaxon.struc.*;

public class CaosNMCEngine{
	...
	public static void main(String[] args)  throws Exception {
		...
		boolean exhaustiveSearch = args.length == 8; // WHAT IS THAT ?
		...
		SearchInterface search; // WHATS THAT ?
		if(!exhaustiveSearch) {
			System.err.println("Proofing.");
			search = new NestedMonteCarlo();
		} else { // WHAT'S THAT ?
			System.err.println("Exhausting.");
			search = new ExhaustiveSearch();
		}

		search.initializeSearch(ol, sml, bound); // TODO : to implement in NestedMonteCarlo
		search.search(initialState); // TODO : to implement in NestedMonteCarlo
